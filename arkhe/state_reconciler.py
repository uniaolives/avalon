...
class StateReconciler:
    ...
    def _weighted_merge_content(self, weighted_states: List[tuple]) -> Any:
        ...
        if isinstance(contents[0], (int, float)):
            ...
            return sum(c * w for c, w in zip(contents, weights)) / total_weight if total_weight > 0 else 0

        # Para estruturas complexas, usar provedor de maior confiança
        max_idx = weights.index(max(weights))
        return contents[max_idx]

    def _consensus_merge(self, states, C, F):
        # Implementation of consensus merge
        return {"content": states[0].content, "metadata": {"strategy": "consensus", "coherence": C}}

    def _priority_merge(self, states):
